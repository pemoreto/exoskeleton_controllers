...
def create_splines(cp_idx_r, cp_ang_r, cp_idx_l, cp_ang_l):
    ...
    x_middle_r = np.arange(0, time_cycle[-1] + 0.01, 0.01)  # Generate discrete time values
    ...
    x_edge_r = np.arange(time_cycle[cp_idx_r[-1]], time_cycle[-1] + time_cycle[cp_idx_r[0]] + 1, 0.01)
    y_edge_r = spline_edge_r(x_edge_r)
    # Determine the end of the transition region
    idx_endpoint_edge_r = np.searchsorted(x_edge_r, time_cycle[-1] + time_cycle[cp_idx_r[0]])
    x_edge_r = x_edge_r[1:idx_endpoint_edge_r]
    y_edge_r = y_edge_r[1:idx_endpoint_edge_r]
    ...
    x_edge_l = np.arange(time_cycle[cp_idx_l[-1]], time_cycle[-1] + time_cycle[cp_idx_l[0]] + 1, 0.01)
    y_edge_l = spline_edge_l(x_edge_l)
    idx_endpoint_edge_l = np.searchsorted(x_edge_l, time_cycle[-1] + time_cycle[cp_idx_l[0]])
    x_edge_l = x_edge_l[1:idx_endpoint_edge_l]
    y_edge_l = y_edge_l[1:idx_endpoint_edge_l]

    # Split the edge region into two sections (right side)
    split_idx_r = np.searchsorted(x_edge_r, time_cycle[-1])
    x_edge_1_r = x_edge_r[:split_idx_r + 1]
    y_edge_1_r = y_edge_r[:split_idx_r + 1]
    x_edge_2_r = x_edge_r[split_idx_r:] - time_cycle[-1]  # Reset time for continuity
    y_edge_2_r = y_edge_r[split_idx_r:]

    # Split the edge region into two sections (left side)
    split_idx_l = np.searchsorted(x_edge_l, time_cycle[-1])
    x_edge_1_l = x_edge_l[:split_idx_l + 1]
    y_edge_1_l = y_edge_l[:split_idx_l + 1]
    x_edge_2_l = x_edge_l[split_idx_l:] - time_cycle[-1]
    y_edge_2_l = y_edge_l[split_idx_l:]
